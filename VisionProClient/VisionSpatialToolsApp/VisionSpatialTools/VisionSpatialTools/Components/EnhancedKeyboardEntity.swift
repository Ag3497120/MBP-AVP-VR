    ...
    private func buildKeyboard() {
        ...
        for (rowIndex, row) in keyLayout.enumerated() {
            ...
            for (colIndex, letter) in row.enumerated() {
                ...
                let (entity, keyModel) = makeKey(letter: letter, width: keyWidth, position: pos)
                addChild(entity)
                keyEntities[letter] = keyModel
        ...
        let (spaceEntity, spaceModel) = makeKey(letter: "SPACE", width: spaceWidth, position: spacePos)
        addChild(spaceEntity)
        keyEntities["SPACE"] = spaceModel
        allKeyDescs.append(KeyDescriptor(
            letter: "SPACE",
        ...
        let (bsEntity, bsModel) = makeKey(letter: "⌫", width: keyWidth * 1.5, position: bsPos, accent: true)
        addChild(bsEntity)
        keyEntities["⌫"] = bsModel
        allKeyDescs.append(KeyDescriptor(
            letter: "⌫",
        ...
        let (enterEntity, enterModel) = makeKey(letter: "⏎", width: keyWidth * 1.5, position: enterPos, accent: true)
        addChild(enterEntity)
        keyEntities["⏎"] = enterModel
        allKeyDescs.append(KeyDescriptor(
            letter: "⏎",
