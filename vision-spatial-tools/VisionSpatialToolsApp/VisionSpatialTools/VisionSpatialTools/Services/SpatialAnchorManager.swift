...
class SpatialAnchorManager: ObservableObject {
    @Published var anchors: [UInt64: AnchorEntity] = [:]
    ...
    func moveAnchor(id: UInt64, to position: SIMD3<Float>) {
        if let anchor = anchors[id] {
            anchor.position = position
        }
    }

    func getAnchorTransform(id: UInt64) -> simd_float4x4? {
